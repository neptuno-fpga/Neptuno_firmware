...
begin
  ...
  if rising_edge(Clk) then
    ...
    if KbdInt = '1' then
			...
			-- delgrom unamiga
			if KbdScanCode = "01111110" then fn_pulse(1) <= not(IsReleased); end if; -- ScrLock : 0x7E					
			
			if KbdScanCode = x"05" 
			then 
				btn_one_player    <= not(IsReleased); 
				fn_pulse(2)   <= not(IsReleased); 				
			end if; -- F1			
			
			
			if KbdScanCode = x"06" then 
				btn_two_players   <= not(IsReleased);
				fn_pulse(3)   <= not(IsReleased); 				
			end if; -- F2
			...
			if KbdScanCode = x"76" 
			then 
				btn_coin          <= not(IsReleased); 
				fn_pulse(0)			<= not(IsReleased); 
			end if; -- ESC
